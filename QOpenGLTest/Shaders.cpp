...
const char* shaders::vertexShaderTR = R"(#version 330 core
layout (location = 0) in vec3 aPos;
layout (location = 1) in vec3 aColor;
layout (location = 2) in vec2 aTexture;
...
void main()
{
    ...
    outColor = vec4(aColor, 1.0);
    outTexture = vec2(aTexture.x, aTexture.y);
}
)";

const char* shaders::fragmentShaderTR = R"(#version 330 core
out vec4 fragColor;
...
const char* shaders::vertexShader3D = R"(#version 330 core
layout (location = 0) in vec3 aPos;
...
const char* shaders::fragmentShader3D = R"(#version 330 core
out vec4 fragColor;
in vec2 outTexture;
